...
public class GameController implements Initializable {
	...
	public void updateUI() {
		...
		for (int i = 0; i < 2; i++) {  //To check index[0] & index[gameLimitX-1]
			for (int y = 2; y < Game.getLimitY(); y++) { // Given that the first two rows are shark free, no need to check
				if (i == 0) { //at left grid boundry
					...
					if (((ImageView) hboxRoom.lookup("#x" + (Game.getLimitX() - 1) + "y" + y)).getImage() == imageHash.get("sharkRight")) { //and there's a shark going right
						if (App.game.getGrid()[Game.getLimitX() - 1][y].getCollectable() != null) { //code to check for item
							setImageViewImage((Game.getLimitX() - 1), y, imageHash.get(App.game.getGrid()[Game.getLimitX() - 1][y].getCollectable().getName())); //<-- correct item
						} else {
							setImageViewImage((Game.getLimitX() - 1), y, imageHash.get("emptyWater"));
						}
					}
				}
			}
